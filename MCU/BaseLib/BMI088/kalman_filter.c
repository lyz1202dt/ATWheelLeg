...
#define KALMAN_MATRIX_SIZE_MISMATCH -1
#define KALMAN_MATRIX_SINGULAR -2
#define KALMAN_MATRIX_MAX_DIM 6

/* 保证 mat 与 arm_matrix_instance_f32 内存布局完全一致，从而可安全地相互转换 */
_Static_assert(sizeof(mat) == sizeof(arm_matrix_instance_f32),
               "mat layout must match arm_matrix_instance_f32");
...
void Matrix_Init(mat *matrix, uint16_t rows, uint16_t cols, float *data)
{
    if (matrix == NULL) {
        return;
    }

    arm_mat_init_f32((arm_matrix_instance_f32 *)matrix, rows, cols, data);
}

int8_t Matrix_Add(const mat *src_a, const mat *src_b, mat *dst)
{
    if (src_a == NULL || src_b == NULL || dst == NULL ||
        src_a->pData == NULL || src_b->pData == NULL || dst->pData == NULL ||
        src_a->numRows != src_b->numRows || src_a->numCols != src_b->numCols ||
        dst->numRows != src_a->numRows || dst->numCols != src_a->numCols) {
        return KALMAN_MATRIX_SIZE_MISMATCH;
    }

    const arm_status status = arm_mat_add_f32((const arm_matrix_instance_f32 *)src_a,
                                              (const arm_matrix_instance_f32 *)src_b,
                                              (arm_matrix_instance_f32 *)dst);
    return (status == ARM_MATH_SUCCESS) ? KALMAN_MATRIX_OK : KALMAN_MATRIX_SIZE_MISMATCH;
}

int8_t Matrix_Subtract(const mat *src_a, const mat *src_b, mat *dst)
{
    if (src_a == NULL || src_b == NULL || dst == NULL ||
        src_a->pData == NULL || src_b->pData == NULL || dst->pData == NULL ||
        src_a->numRows != src_b->numRows || src_a->numCols != src_b->numCols ||
    ...
    const arm_status status = arm_mat_sub_f32((const arm_matrix_instance_f32 *)src_a,
                                              (const arm_matrix_instance_f32 *)src_b,
                                              (arm_matrix_instance_f32 *)dst);
    return (status == ARM_MATH_SUCCESS) ? KALMAN_MATRIX_OK : KALMAN_MATRIX_SIZE_MISMATCH;
}

int8_t Matrix_Multiply(const mat *src_a, const mat *src_b, mat *dst)
{
    if (src_a == NULL || src_b == NULL || dst == NULL ||
        src_a->pData == NULL || src_b->pData == NULL || dst->pData == NULL ||
        src_a->numCols != src_b->numRows ||
        dst->numRows != src_a->numRows || dst->numCols != src_b->numCols) {
        return KALMAN_MATRIX_SIZE_MISMATCH;
    }

    const arm_status status = arm_mat_mult_f32((const arm_matrix_instance_f32 *)src_a,
                                               (const arm_matrix_instance_f32 *)src_b,
                                               (arm_matrix_instance_f32 *)dst);
    return (status == ARM_MATH_SUCCESS) ? KALMAN_MATRIX_OK : KALMAN_MATRIX_SIZE_MISMATCH;
}

int8_t Matrix_Transpose(const mat *src, mat *dst)
{
    if (src == NULL || dst == NULL ||
        src->pData == NULL || dst->pData == NULL ||
        dst->numRows != src->numCols || dst->numCols != src->numRows) {
        return KALMAN_MATRIX_SIZE_MISMATCH;
    }

    const arm_status status = arm_mat_trans_f32((const arm_matrix_instance_f32 *)src,
                                                (arm_matrix_instance_f32 *)dst);
    return (status == ARM_MATH_SUCCESS) ? KALMAN_MATRIX_OK : KALMAN_MATRIX_SIZE_MISMATCH;
}

int8_t Matrix_Inverse(const mat *src, mat *dst)
{
    if (src == NULL || dst == NULL ||
        src->pData == NULL || dst->pData == NULL ||
        src->numRows != src->numCols ||
        dst->numRows != src->numRows || dst->numCols != src->numCols ||
        src->numRows > KALMAN_MATRIX_MAX_DIM) {
        return KALMAN_MATRIX_SIZE_MISMATCH;
    }

    const arm_status status = arm_mat_inverse_f32((const arm_matrix_instance_f32 *)src,
                                                  (arm_matrix_instance_f32 *)dst);
    if (status == ARM_MATH_SUCCESS) {
        return KALMAN_MATRIX_OK;
    } else if (status == ARM_MATH_SINGULAR) {
        return KALMAN_MATRIX_SINGULAR;
    }
    return KALMAN_MATRIX_SIZE_MISMATCH;
}
